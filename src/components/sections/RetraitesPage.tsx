...
import { ArrowRight, Bell, Check, Mountain, X } from 'lucide-react';
...
import { evenementsRetraitesCopy } from '@/data/copy/evenementsRetraites';
import { submitWaitlist } from '@/lib/api/waitlist';
import { useT } from '@/lib/i18n/useT';

const CAPTURE_ANCHOR_ID = 'retraites-notify';

/**
 * RetraitesPage — /evenements/retraites (FR) + /en/events/retreats (EN).
 *
 * Skeleton pré-lancement honnête. Sage palette + mountain peak signature
 * distinguishes from gold/tactical bootcamps + indigo/digital masterclass.
 *
 * 6 sections :
 *   1. Hero (eyebrow + h1 + sub + microProof + mountain glyph signature)
 *   2. Pitch (4 piliers : architecture / lieu / groupe / accès Jonas)
 *   3. Pour qui / pas pour qui (5 + 3 critères)
 *   4. Format (5 rows : durée / lieu / audience / animation / tarif / édition)
 *   5. Notify capture email (wired /api/waitlist source="retraites-hero")
 *   6. Cross-link footer (3 cards Bootcamps · Retraites current · Masterclass)
 *
 * Pas de Stripe câblé (pré-lancement). Le seul CTA actif = capture email.
 * Anti-cannibalisation respectée : CTA primaire "Sois notifié" reste
 * uniquement à l'intérieur de cette sous-page.
 */
export function RetraitesPage() {
  const { t, locale } = useT();
  const copy = evenementsRetraitesCopy;
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setError(null);
    const route =
      typeof window !== 'undefined' ? window.location.pathname : '/evenements/retraites';
  ...
  return (
    <BootcampThemeProvider variant="retraite">
      {/* ─── 1. HERO ──────────────────────────────────────────────────── */}
      <section
      ...
      >
        ...
        {/* Mountain peak signature — sage SVG layered behind hero */}
        <div
          aria-hidden="true"
        ...
        >
          ...
          >
            <defs>
              ...
              >
                {/* Sage peaks — successive ridge lines */}
                <path
                  d="M 0 130 L 55 80 L 110 115 L 165 60 L 220 100 L 220 160 L 0 160 Z"
                  fill="none"
                  stroke="var(--bc-pattern-stroke)"
                  strokeWidth="0.8"
                />
                <path
                  d="M 0 145 L 40 110 L 90 130 L 140 95 L 195 125 L 220 110 L 220 160 L 0 160 Z"
                  fill="none"
                  stroke="var(--bc-pattern-stroke)"
                  strokeWidth="0.6"
                  opacity="0.7"
            ...
            <rect width="100%" height="100%" fill="url(#retraites-peaks)" />
          </svg>
        </div>
          ...
          <div className="relative max-w-default mx-auto px-md">
            ...
            >
              <div className="flex flex-col sm:flex-row gap-2 sm:items-stretch">
                <label htmlFor="retraites-email" className="sr-only">
                ...
                />
                {/* Honeypot — aria-hidden on input directly to satisfy axe */}
                <input
                  id="retraites-website"
                  name="website"
                  type="text"
                  tabIndex={-1}
                  autoComplete="off"
        ...
        >
          ...
          <div className="relative max-w-default mx-auto px-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-md">
              ...
              <article className="flex flex-col gap-2 p-md bg-elevated border border-[oklch(0.74_0.05_155)]/55 rounded-lg shadow-haptic-focal relative">
                <span className="absolute top-sm right-sm text-eyebrow uppercase tracking-widest text-[oklch(0.74_0.05_155)] font-display text-[10px]">
              ...
              <Link
                ...
                className="group flex flex-col gap-2 p-md bg-elevated border border-[oklch(0.75_0.1_270)]/25 hover:border-[oklch(0.75_0.1_270)]/55 rounded-lg shadow-haptic-card transition-colors duration-base"
              >
                ...
                <span className="mt-auto pt-sm inline-flex items-center gap-2 text-eyebrow uppercase tracking-widest text-[oklch(0.75_0.1_270)] font-display text-xs">
                  {locale === 'fr' ? 'Découvrir' : 'Discover'}
                  <ArrowRight className="h-3.5 w-3.5 max-w-none shrink-0" aria-hidden="true" />
                </span>
              </Link>
